		...
		u8 **path_p)
{
	...
	dev_err(&client->dev, "fzh1-1 gup_check_firmware_name \n");
				...
				struct st_fw_head *fw_head, u8 *path)
{
	...
	dev_err(&client->dev, "fzh1 gup_get_update_file \n");

	ret = gup_check_firmware_name(client, &path);
	if (ret < 0)
		return FAIL;
	dev_err(&client->dev, "fzh2 gup_get_update_file \n");
	...
	dev_err(&client->dev, "fzh3 gup_get_update_file \n");
